import { GET_ALL_DEVELOPERS, GET_ALL_DEVELOPERS_FAILED , GET_ALL_CHATS_USER_WISE , GET_ALL_CHATS_USER_WISE_FAILED, GET_ALL_INVESTORS } from '../ActionTypes/UserActionTypes'
...
export const getAllInvestors = (token) => async (dispatch) => {
    try {
        const data = await getData("/api/getAllInvestors", {}, token);
        console.log("got it", data.data)
        dispatch({ type: GET_ALL_INVESTORS, payload: data.data });
        return data;
    } catch (error) {
        dispatch({ type: GET_ALL_DEVELOPERS_FAILED, payload: error.message });
    }
